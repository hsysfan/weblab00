...
<html>
	...
	<body>
		...
		<ul> 
			...
			<li>Course: <?= processCheckbox('check')?></li>
			<li>Grade : <?= $_POST['grade']; ?></li>
			<li>Credit : <?= $_POST['cnumber'].' ('.$_POST['card']; ?>)</li>
		</ul>
		...
		<?php
			 ...
			 */
			$info = $_POST['name'].';'.$_POST['id'].';'.$_POST['cnumber'].';'.$_POST['card']."\n";
		...
		<?php
			...
			function processCheckbox(){
				$allchecked ='';
				$count = 0;
				foreach($_POST['check'] as $input){
					$count++;
					if($count == 1){
						$allchecked = $input;
					}
					else{
						$allchecked = $allchecked.",".$input;
						}
					}
				 return $allchecked;
				}
