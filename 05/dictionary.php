...
<!-- Ex. 1: File of Dictionary -->
    <?php
        ...
        print"My dictionary has ".$linescount." total words
        and
...
<!-- Ex. 2: Today’s Words & Ex 6: Query Parameters -->
        <?php
            function getWordsByNumber($listOfWords, $numberOfWords){
                $resultArray = array();
                shuffle($listOfWords);
                $resultArray = array_slice($listOfWords,0,$numberOfWords);
                return $resultArray;
            }
            $twords = getWordsByNumber($lines,4);
        ?>
        <ol>
            <?php foreach ($twords as $tmp) {
                $temp = explode("\t", $tmp);
                ?>
                <li><?= $temp[0]." - ".$temp[1] ?></li>
           <?php }?>
...
<!-- Ex. 3: Searching Words & Ex 6: Query Parameters -->
        <?php
            ...
            getWordsByCharacter($lines, $startCharacter);
        ?>
        <p>
